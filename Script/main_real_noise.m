...
file_name = "song_real_noise4";
file_to_read = 'Music1_bensound-happyrock.mp3';
file_noise = "Noise4_TV_2.wav";
...
for i=1:size(snr_array)
    ...
    %*** Real Noise
    [n,Fs] = audioread(file_noise);
    if(Fs~=48000)
        s=resample(s,48000,Fs);
        Fs=48000;
    end    
    n=n(:,1); %wav from iphone returns two column-> two channels. get only one.
    T=1/Fs; %Period
    N=ceil(length*Fs); %Amount of samples to length
    n=n(1:min(size(n,1), N));
    n=normalize(n);
    n = n.*10^((power_s_db-snr)/20); %increase the power by the snr factor
    power_n = rms(n)^2;
    power_n_db = pow2db(power_n);
    if size(s,1)~=size(n,1) 
        return
    end
